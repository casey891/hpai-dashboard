...
COLORS = {
    "Commercial Table Egg Layer": "#dc2626",
    "Commercial Table Egg Pullets": "#f87171",
    "Commercial Broiler Production": "#ea580c",
    "Commercial Broiler Breeder": "#fb923c",
    "Commercial Turkey Meat Bird": "#ca8a04",
    "Commercial Turkey Breeder Hens": "#facc15",
    "Commercial Duck Meat Bird": "#16a34a",
    "Commercial Duck Breeder": "#4ade80",
    "Commercial Upland Gamebird Producer": "#2dd4bf",
    "Commercial Raised for Release Upland Game Bird": "#5eead4",
    "Commercial Raised for Release Waterfowl": "#99f6e4",
    "Commercial Breeder Operation": "#0ea5e9",
    "Commercial Breeder (Multiple Bird Species)": "#38bdf8",
    "Commercial Turkey Breeder Replacement Hens": "#fde047",
    "Commercial Turkey Breeder Toms": "#fef08a",
    "Commercial Turkey Poult Supplier": "#fef9c3",
    "Commercial Table Egg Breeder": "#fca5a5",
    "Commercial Broiler Breeder Pullets": "#fdba74",
    "Primary Broiler Breeder Pedigree Farm": "#7dd3fc",
    "Live Bird Market": "#9333ea",
    "Live Bird Sales  (non-slaughter)": "#a78bfa",
    "WOAH Poultry": "#6366f1",
    "WOAH Non-Poultry": "#818cf8",
}
...
body{font-family:'Lexend',sans-serif;background:#0f1117;color:#e2e8f0;line-height:1.5}
.wrap{max-width:1100px;margin:0 auto;padding:20px 16px}
header{text-align:center;margin-bottom:20px}
header h1{font-size:1.6rem;font-weight:700;color:#f1f5f9}
header .sub{color:#8892a4;font-size:.8rem;margin-top:2px}
.kpi-row{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin-bottom:20px}
.kpi{background:#1a1d27;border-radius:10px;padding:14px 18px;box-shadow:0 1px 3px rgba(0,0,0,.3)}
.kpi .lbl{font-size:.7rem;text-transform:uppercase;letter-spacing:.04em;color:#8892a4;font-weight:600}
.kpi .val{font-size:1.4rem;font-weight:700;margin-top:2px}
.kpi .note{font-size:.7rem;color:#64748b;margin-top:1px}
.card{background:#1a1d27;border-radius:10px;padding:20px;margin-bottom:14px;box-shadow:0 1px 3px rgba(0,0,0,.3)}
.card h2{font-size:1rem;font-weight:600;margin-bottom:2px;color:#f1f5f9}
.card .sub{font-size:.78rem;color:#8892a4;margin-bottom:12px}
footer{text-align:center;padding:14px;color:#64748b;font-size:.7rem}
...
.rbtn{padding:3px 10px;border:1px solid #2d3344;border-radius:16px;background:#252833;font-size:.72rem;cursor:pointer;color:#94a3b8;transition:all .15s}
.rbtn:hover{background:#2d3344}
.rbtn.active{background:#2563eb;color:#fff;border-color:#2563eb}
/* multi-select */
.ms-wrap{position:relative;display:inline-block}
.ms-btn{padding:5px 12px;border:1px solid #2d3344;border-radius:6px;background:#252833;font-size:.8rem;cursor:pointer;color:#cbd5e1;min-width:160px;text-align:left}
.ms-btn::after{content:'▾';float:right;margin-left:8px;color:#64748b}
.ms-panel{display:none;position:absolute;right:0;z-index:20;background:#1a1d27;border:1px solid #2d3344;border-radius:8px;padding:8px 10px;max-height:320px;overflow-y:auto;min-width:290px;box-shadow:0 4px 16px rgba(0,0,0,.4);margin-top:4px}
.ms-panel.open{display:block}
.ms-actions{padding:2px 0 6px;border-bottom:1px solid #1e2130;margin-bottom:6px;font-size:.75rem}
.ms-actions a{color:#3b82f6;text-decoration:none;cursor:pointer}
.ms-actions a:hover{text-decoration:underline}
.ms-item{display:flex;align-items:center;padding:3px 0;cursor:pointer;font-size:.8rem;gap:6px;color:#cbd5e1}
.ms-item input{margin:0;cursor:pointer}
.ms-dot{width:10px;height:10px;border-radius:2px;flex-shrink:0}
/* data table */
.tbl-wrap{max-height:400px;overflow-y:auto;border:1px solid #2d3344;border-radius:6px;margin-top:8px}
.tbl-wrap table{width:100%;border-collapse:collapse;font-size:.78rem}
.tbl-wrap thead{position:sticky;top:0;z-index:2}
.tbl-wrap th{background:#252833;padding:7px 10px;text-align:left;font-weight:600;color:#94a3b8;border-bottom:2px solid #2d3344;white-space:nowrap}
.tbl-wrap td{padding:5px 10px;border-bottom:1px solid #1e2130;color:#cbd5e1}
.tbl-wrap tr:nth-child(even) td{background:#252833}
.tbl-wrap .num{text-align:right;font-variant-numeric:tabular-nums}
.tbl-summary{font-size:.75rem;color:#8892a4;margin-top:6px}
.ms-group{margin-bottom:2px}
.ms-group-hdr{display:flex;align-items:center;padding:5px 0 3px;cursor:pointer;font-size:.8rem;font-weight:600;gap:6px;color:#e2e8f0}
.ms-group-hdr input{margin:0;cursor:pointer}
.ms-group-children{padding-left:20px}
/* tabs */
.tab-bar{display:flex;gap:0;margin-bottom:14px;border-bottom:2px solid #2d3344;overflow-x:auto}
...
.county{stroke:#2d3344;stroke-width:.25px;transition:opacity .12s}
.county:hover{opacity:.8;stroke:#f1f5f9;stroke-width:1px}
.state-border{fill:none;stroke:#4a5568;stroke-width:.7px;pointer-events:none}
.map-tooltip{position:absolute;pointer-events:none;background:#1a1d27;border:1px solid #2d3344;border-radius:8px;padding:10px 14px;box-shadow:0 4px 12px rgba(0,0,0,.4);font-size:.8rem;max-width:280px;z-index:30;opacity:0;transition:opacity .12s}
.map-tooltip.visible{opacity:1}
.tt-county{font-weight:700;font-size:.9rem;color:#f1f5f9}
.tt-total{font-size:1.1rem;font-weight:700;margin:4px 0;color:#ef4444}
.tt-row{font-size:.75rem;color:#94a3b8;line-height:1.6}
.tt-date{font-size:.7rem;color:#64748b;margin-top:4px}
.legend-wrap{display:flex;align-items:center;gap:6px;font-size:.7rem;color:#8892a4}
.legend-bar{width:200px;height:12px;border-radius:3px}
@keyframes spin{to{transform:rotate(360deg)}}
#loadingOverlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(15,17,23,.95);z-index:1000;display:flex;align-items:center;justify-content:center}
</style>
</head>
<body>
<div id="loadingOverlay">
  <div style="text-align:center">
    <div style="width:40px;height:40px;border:3px solid #2d3344;border-top-color:#2563eb;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 12px"></div>
    <div style="font-size:.9rem;color:#94a3b8;font-weight:600">Loading dashboard data&hellip;</div>
  </div>
</div>
...
<div class="kpi-row">
  <div class="kpi">
    ...
    <div class="val" style="color:#dc2626" id="kpiLayers">&mdash;</div>
    <div class="note">Commercial egg layers, last 30 days</div>
  </div>
  <div class="kpi">
    <div class="lbl">Poultry Infection Sites</div>
    <div class="val" style="color:#f59e0b" id="kpiSites">&mdash;</div>
    <div class="note">All flock types, last 30 days</div>
  </div>
  <div class="kpi">
    <div class="lbl">Wild Bird Detections</div>
    <div class="val" style="color:#16a34a" id="kpiWB">&mdash;</div>
    <div class="note">Confirmed cases, last 30 days</div>
  </div>
  <div class="kpi">
  ...
  <div class="kpi">
    ...
    <div class="val" style="color:#0ea5e9" id="kpiPrice">&mdash;</div>
...
const MM_COLORS={'Domestic/Companion':'#dc2626','Wild Carnivores':'#ea580c','Rodents/Small Mammals':'#ca8a04','Marine Mammals':'#0ea5e9','Captive/Zoo':'#9333ea','Other':'#6b7280'};

/* ── Chart.js dark defaults ── */
Chart.defaults.color='#94a3b8';
Chart.defaults.borderColor='#2d3344';
Chart.defaults.plugins.tooltip.backgroundColor='#252833';
Chart.defaults.plugins.tooltip.titleColor='#f1f5f9';
Chart.defaults.plugins.tooltip.bodyColor='#cbd5e1';
Chart.defaults.plugins.tooltip.borderColor='#2d3344';
Chart.defaults.plugins.tooltip.borderWidth=1;

/* ── Loading UI ── */
function hideLoading(){document.getElementById('loadingOverlay').style.display='none';}
function showError(msg){
  document.getElementById('loadingOverlay').innerHTML='<div style="text-align:center;color:#ef4444"><div style="font-size:1.2rem;font-weight:700;margin-bottom:8px">Failed to load data</div><div style="font-size:.85rem;color:#8892a4">'+msg+'</div><button onclick="location.reload()" style="margin-top:12px;padding:8px 20px;border:1px solid #2d3344;border-radius:6px;background:#1a1d27;color:#e2e8f0;cursor:pointer">Retry</button></div>';
}
...
function updateMapColors(){
  ...
  countyPaths.attr('fill',function(){const fips=this.getAttribute('data-fips');const count=getFilteredCounts(fips).total;return count>0?scale(count):'#1e2130';});
...
function onMapHover(event,d){
  ...
  if(fc.total>0){
    ...
    html+='<div class="tt-row" style="color:#64748b">No detections in this period</div>';
  }
...
function updateBirds(){
  ...
  if(isDaily(birdsRange)){const idx=dailyIndices(birdsRange);birdsChart.data.labels=sliceByIdx(D.daily_labels,idx);birdsChart.data.datasets=sel.map(p=>({label:p,data:idx.map(i=>(D.daily_birds[D.daily_dates[i]]||{})[p]||0),backgroundColor:D.category_colors[p]||'#6b7280'}));}
  else{const idx=monthIndices(birdsRange);birdsChart.data.labels=sliceByIdx(D.month_labels,idx);birdsChart.data.datasets=sel.map(p=>({label:p,data:idx.map(i=>(D.birds_by_month[D.months[i]]||{})[p]||0),backgroundColor:D.category_colors[p]||'#6b7280'}));}
...
function updateInf(){
  ...
  if(isDaily(infRange)){const idx=dailyIndices(infRange);infChart.data.labels=sliceByIdx(D.daily_labels,idx);infChart.data.datasets=sel.map(p=>({label:p,data:idx.map(i=>(D.daily_infections[D.daily_dates[i]]||{})[p]||0),backgroundColor:D.category_colors[p]||'#6b7280'}));}
  else{const idx=monthIndices(infRange);infChart.data.labels=sliceByIdx(D.month_labels,idx);infChart.data.datasets=sel.map(p=>({label:p,data:idx.map(i=>(D.infections_by_month[D.months[i]]||{})[p]||0),backgroundColor:D.category_colors[p]||'#6b7280'}));}
...
function initWildBirds(){
  ...
  wbChart=new Chart(document.getElementById('cWildBirds'),{type:'bar',data:{labels:WB.month_labels,datasets:[{label:'Detections',data:WB.monthly_counts,backgroundColor:'#16a34a'}]},options:{responsive:true,aspectRatio:2.5,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.parsed.y.toLocaleString()+' detections'}}},scales:{x:{ticks:{maxRotation:45},grid:{display:false}},y:{title:{display:true,text:'Detections'},grid:{color:'#2d3344'},ticks:{callback:fmtBirds}}}}});
  updateWbTable();
}
...
function initMammals(){
  const MM=D.mammals;
  mmChart=new Chart(document.getElementById('cMammals'),{type:'bar',data:{labels:MM.month_labels,datasets:MM.groups.map(g=>({label:g,data:MM.months.map(m=>(MM.monthly_by_group[m]||{})[g]||0),backgroundColor:MM_COLORS[g]||'#6b7280'}))},options:{responsive:true,aspectRatio:2.5,plugins:{legend:{display:true,position:'bottom',labels:{boxWidth:12,font:{size:11},color:'#94a3b8'}},tooltip:{callbacks:{label:c=>c.dataset.label+': '+c.parsed.y}}},scales:{x:{stacked:true,ticks:{maxRotation:45},grid:{display:false}},y:{stacked:true,title:{display:true,text:'Detections'},grid:{color:'#2d3344'}}}}});
  updateMmTable();
}
function updateMammals(){
  ...
  mmChart.data.datasets=MM.groups.map(g=>({label:g,data:idx.map(i=>(MM.monthly_by_group[MM.months[i]]||{})[g]||0),backgroundColor:MM_COLORS[g]||'#6b7280'}));
...
function initDashboard(){
  ...
  eggChart=new Chart(document.getElementById('cEgg'),{type:'line',
    data:{labels:D.egg_dates,datasets:[{label:'Caged Large',data:D.caged_prices,borderColor:'#f59e0b',backgroundColor:'rgba(245,158,11,.15)',fill:true,tension:.2,pointRadius:0,pointHitRadius:8,borderWidth:2}]},
    options:{responsive:true,aspectRatio:2.5,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.parsed.y!=null?'$'+c.parsed.y.toFixed(2)+'/dz':''}}},scales:{x:{ticks:{maxTicksLimit:12,maxRotation:0},grid:{display:false}},y:{title:{display:true,text:'$ / Dozen'},grid:{color:'#2d3344'},ticks:{callback:v=>'$'+v.toFixed(2)}}}}});
  ...
  birdsChart=new Chart(document.getElementById('cBirds'),{type:'bar',
    ...
    options:{responsive:true,aspectRatio:2.5,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>{const v=c.parsed.y;return c.dataset.label+': '+(v>=1e6?(v/1e6).toFixed(2)+'M':v>=1e3?(v/1e3).toFixed(0)+'K':v)+' birds';}}}},scales:{x:{stacked:true,ticks:{maxRotation:45},grid:{display:false}},y:{stacked:true,title:{display:true,text:'Total Birds'},grid:{color:'#2d3344'},ticks:{callback:fmtBirds}}}}});

  infChart=new Chart(document.getElementById('cInf'),{type:'bar',
    data:{labels:D.month_labels,datasets:D.production_types.map(p=>({label:p,data:D.months.map(m=>(D.infections_by_month[m]||{})[p]||0),backgroundColor:D.category_colors[p]||'#6b7280'}))},
    options:{responsive:true,aspectRatio:2.5,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.dataset.label+': '+c.parsed.y+' sites'}}},scales:{x:{stacked:true,ticks:{maxRotation:45},grid:{display:false}},y:{stacked:true,title:{display:true,text:'Number of Sites'},grid:{color:'#2d3344'}}}}});
  ...
  const layerBirds=D.events.filter(e=>e.d>=cut30&&e.p==='Commercial Table Egg Layer').reduce((s,e)=>s+e.f,0);
  document.getElementById('kpiLayers').textContent=fmtBirds(layerBirds);
  const pSites=D.events.filter(e=>e.d>=cut30).length;
  document.getElementById('kpiSites').textContent=pSites.toLocaleString();
  const wbDet=D.wild_birds?D.wild_birds.events.filter(e=>e.d>=cut30).length:0;
  document.getElementById('kpiWB').textContent=wbDet.toLocaleString();
  const cur=pSites+wbDet;
...
def generate_html(data):
  ...
  <div class="card">
    ...
    # ── Tab content: Mammals ──
    if "mammals" in data:
        mm_colors = {'Domestic/Companion': '#dc2626', 'Wild Carnivores': '#ea580c',
                     'Rodents/Small Mammals': '#ca8a04', 'Marine Mammals': '#0ea5e9',
                     'Captive/Zoo': '#9333ea', 'Other': '#6b7280'}
