...
.up{color:#dc2626}.dn{color:#16a34a}
/* range buttons */
...
.ms-btn{padding:5px 12px;border:1px solid #e2e8f0;border-radius:6px;background:#f8fafc;font-size:.8rem;cursor:pointer;color:#334155;min-width:160px;text-align:left}
.ms-btn::after{content:'▾';float:right;margin-left:8px;color:#94a3b8}
...
.tbl-wrap th{background:#f8fafc;padding:7px 10px;text-align:left;font-weight:600;color:#475569;border-bottom:2px solid #e2e8f0;white-space:nowrap}
.tbl-wrap td{padding:5px 10px;border-bottom:1px solid #f1f5f9;color:#334155}
.tbl-wrap tr:nth-child(even) td{background:#f8fafc}
.tbl-wrap .num{text-align:right;font-variant-numeric:tabular-nums}
.tbl-summary{font-size:.75rem;color:#64748b;margin-top:6px}
.ms-group{margin-bottom:2px}
.ms-group-hdr{display:flex;align-items:center;padding:5px 0 3px;cursor:pointer;font-size:.8rem;font-weight:600;gap:6px;color:#1e293b}
...
<div id="loadingOverlay">
  <div style="text-align:center">
    <div style="width:40px;height:40px;border:3px solid #e2e8f0;border-top-color:#1e3a5f;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 12px"></div>
    <div style="font-size:.9rem;color:#475569;font-weight:600">Loading dashboard data&hellip;</div>
  </div>
</div>
...
function showError(msg){
  document.getElementById('loadingOverlay').innerHTML='<div style="text-align:center;color:#dc2626"><div style="font-size:1.2rem;font-weight:700;margin-bottom:8px">Failed to load data</div><div style="font-size:.85rem;color:#64748b">'+msg+'</div><button onclick="location.reload()" style="margin-top:12px;padding:8px 20px;border:1px solid #e2e8f0;border-radius:6px;background:#fff;cursor:pointer">Retry</button></div>';
}
...
function initMammals(){
  const MM=D.mammals;
  mmChart=new Chart(document.getElementById('cMammals'),{type:'bar',data:{labels:MM.month_labels,datasets:MM.groups.map(g=>({label:g,data:MM.months.map(m=>(MM.monthly_by_group[m]||{})[g]||0),backgroundColor:MM_COLORS[g]||'#6b7280'}))},options:{responsive:true,aspectRatio:2.5,plugins:{legend:{display:true,position:'bottom',labels:{boxWidth:12,font:{size:11}}},tooltip:{callbacks:{label:c=>c.dataset.label+': '+c.parsed.y}}},scales:{x:{stacked:true,ticks:{maxRotation:45},grid:{display:false}},y:{stacked:true,title:{display:true,text:'Detections'},grid:{color:'#f1f5f9'}}}}});
  updateMmTable();
}
...
function initDashboard(){
  ...
  eggChart=new Chart(document.getElementById('cEgg'),{type:'line',
    data:{labels:D.egg_dates,datasets:[{label:'Caged Large',data:D.caged_prices,borderColor:'#f59e0b',backgroundColor:'rgba(245,158,11,.06)',fill:true,tension:.2,pointRadius:0,pointHitRadius:8,borderWidth:2}]},
