...
function getFilteredPoultryBreakdown(info,cutM){
  const breakdown={};
  if(!info||!info.mpp)return breakdown;
  for(const[month,prodMap]of Object.entries(info.mpp)){
    if(month<cutM)continue;
...
function getFilteredCounts(fips){
  ...
  const poultryBreakdown=getFilteredPoultryBreakdown(info,cutM);
  let total;
