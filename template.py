...
.map-tooltip{position:absolute;pointer-events:none;background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 14px;box-shadow:0 4px 12px rgba(0,0,0,.12);font-size:.8rem;max-width:360px;z-index:30;opacity:0;transition:opacity .12s}
...
.tt-subhead{font-size:.7rem;font-weight:700;letter-spacing:.02em;text-transform:uppercase;color:#013046;margin-top:8px}
.tt-breakdown{margin-top:4px}
.tt-breakdown-item{padding-top:4px;border-top:1px solid #f1f5f9}
.tt-breakdown-item:first-child{padding-top:0;border-top:none}
.tt-breakdown-label{font-size:.74rem;font-weight:600;color:#334155;line-height:1.35}
.tt-breakdown-meta{font-size:.7rem;color:#64748b;line-height:1.45}
...
<div class="tab-content active" id="tab-poultry">
  ...
  <div class="card">
    ...
    <div class="card-source">Chart: Innovate Animal Ag · Source: <a href="https://www.aphis.usda.gov/livestock-poultry-disease/avian/avian-influenza/hpai-detections/commercial-backyard-flocks" target="_blank">USDA APHIS</a> · <a href="__DATA_URL__/poultry_detections.csv" download>Download Data</a></div>
    <div class="card-source" style="margin-top:6px">APHIS <a href="https://www.aphis.usda.gov/sites/default/files/webinar-prevention-to-recovery-sept2022-qas.pdf" target="_blank">commercial flock thresholds</a> (2022 Q&amp;A): &ge;75,000 table egg layers · &ge;100,000 broilers/yr · &ge;5,000 breeder poultry · &ge;30,000 turkeys/yr · &ge;50,000 gamebirds or waterfowl/yr</div>
    <div class="card-source">Flocks smaller than these thresholds are designated as WOAH. &ldquo;Non-poultry&rdquo; refers to flocks kept by people who do not sell poultry products from that flock.</div>
  </div>
</div>
...
function sumDays(dayDict,cutD){let n=0;for(const[day,cnt]of Object.entries(dayDict)){if(day>=cutD)n+=cnt;}return n;}
function getFilteredPoultryBreakdown(info,cutD,cutM){
  const breakdown={};
  const dailyBreakdown=info&&info.dpp;
  const sourceBreakdown=dailyBreakdown||info&&info.mpp;
  if(!sourceBreakdown)return breakdown;
  for(const[bucket,prodMap]of Object.entries(sourceBreakdown)){
    if(dailyBreakdown?(bucket<cutD):(bucket<cutM))continue;
    for(const[production,vals]of Object.entries(prodMap||{})){
      if(!breakdown[production])breakdown[production]={count:0,birds:0};
      breakdown[production].count+=(vals&&vals[0])||0;
      breakdown[production].birds+=(vals&&vals[1])||0;
    }
  }
  return breakdown;
}
function getFilteredCounts(fips){
  ...
  if(!info)return{total:0,wb:0,p:0,poultryBreakdown:{}};
  const cutD=mapCutoffISO(mapRange);
  const cutM=cutD.slice(0,7);
  const wb=info.dwb?sumDays(info.dwb,cutD):sumMonths(info.mwb||{},cutM);
  const p=info.dp?sumDays(info.dp,cutD):sumMonths(info.mp||{},cutM);
  const poultryBreakdown=getFilteredPoultryBreakdown(info,cutD,cutM);
  let total;
  if(sourceFilter==='wild_birds')total=wb;
  else if(sourceFilter==='poultry')total=p;
  else total=wb+p;
  return{total,wb,p,poultryBreakdown};
}
...
function updateMapColors(){
  ...
  let unk=0;
  if(D.unknown_by_day){
    for(const[day,v]of Object.entries(D.unknown_by_day)){
      if(day>=cutD){
        if(sourceFilter==='wild_birds')unk+=v.wb||0;
        else if(sourceFilter==='poultry')unk+=v.p||0;
        else unk+=(v.wb||0)+(v.p||0);
      }
    }
  } else if(D.unknown_by_month){
    for(const[m,v]of Object.entries(D.unknown_by_month)){
      if(m>=cutM){
        if(sourceFilter==='wild_birds')unk+=v.wb||0;
        else if(sourceFilter==='poultry')unk+=v.p||0;
        else unk+=(v.wb||0)+(v.p||0);
...
function onMapHover(event,d){
  ...
  if(fc.total>0){
    ...
    html+='</div>';
    if(fc.p>0&&sourceFilter!=='wild_birds'){
      const poultryRows=Object.entries(fc.poultryBreakdown)
        .sort((a,b)=>(b[1].count-a[1].count)||(b[1].birds-a[1].birds)||a[0].localeCompare(b[0]));
      if(poultryRows.length){
        html+='<div class="tt-subhead">Poultry breakdown</div><div class="tt-breakdown">';
        poultryRows.forEach(([production,vals])=>{
          html+='<div class="tt-breakdown-item"><div class="tt-breakdown-label">'+production+'</div><div class="tt-breakdown-meta">'+vals.count.toLocaleString()+' infection'+(vals.count!==1?'s':'')+' · '+vals.birds.toLocaleString()+' birds affected</div></div>';
        });
        html+='</div>';
      }
    }
